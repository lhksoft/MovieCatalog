...
// name of Table
#define t3Screen				wxT("tblScreen")
// Required Field-definitions
#define t3Screen_Format			wxT("Format")

class TScreen : public lkSQL3TableSet
{
public:
	TScreen();
	TScreen(lkSQL3Database* pDB);
...
public:
	wxString					GetScreenValue(void) const;
	void						SetScreenValue(const wxString&); // NULL not allowed
...
public:
	...
	static bool					IsUniqueScreen(lkSQL3Database* _DB, const wxString& orig, void*);

	static void					SetDefaultScreen(wxUint64);
	static wxUint64				GetDefaultScreen(lkSQL3Database*); // if valid pDB, a check will be done whether the default exists as a valid rowid
...
private:
	wxDECLARE_DYNAMIC_CLASS_NO_COPY(TScreen);
};
